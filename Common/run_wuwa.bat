...
powershell -NoProfile -Command "$lines = Get-Content 'Engine.ini'; $keep = $false; $corelog = $false; $renderer = $false; $out = @(); foreach ($line in $lines) { if ($line -ieq '[SystemSettings]' -or $line -ieq '[Core.Log]') { $keep = $true; if ($line -ieq '[Core.Log]') { $corelog = $true } }; if ($line -ieq '[/Script/Engine.RendererSettings]') { $renderer = $true; $keep = $false; $out += $line; continue }; if ($keep) { if ($line -eq '' -and $corelog) { $keep = $false; $corelog = $false; continue }; $out += $line } elseif ($renderer) { if ($line -ilike 'r.RayTracing.LoadConfig*') { $out += $line }; if ($line -eq '') { $renderer = $false } } }; $out | Set-Content 'Engine.ini'"
